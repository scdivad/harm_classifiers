...
def get_args():
    parser = argparse.ArgumentParser(
        description="Fine-tune a transformer on the Aegis 2.0 4-class safety dataset."
    )
    ...
    parser.add_argument("--dataset", type=str, required=True,
                        help="Path to DatasetDict saved by build_aegis_v2_dataset.py")
...
def compute_metrics(eval_pred):
    ...
    return {"accuracy": float(acc), "binary_accuracy": float(binary_acc)}
...
def train_sbert(args):
    ...
    eval_texts = [str(t) for t in dataset[eval_split]["text"]]
    train_enc = tokenizer(train_texts, padding=True, truncation=True,
                          max_length=args.max_length, return_tensors="pt")
    eval_enc = tokenizer(eval_texts, padding=True, truncation=True,
                         max_length=args.max_length, return_tensors="pt")
    ...
    for i, name in enumerate(LABEL_NAMES):
        mask = all_labels == i
        if mask.sum() > 0:
            print(f"  {name}: {(all_preds[mask] == i).mean():.2%} ({mask.sum()} examples)")
    print("=" * 40)
